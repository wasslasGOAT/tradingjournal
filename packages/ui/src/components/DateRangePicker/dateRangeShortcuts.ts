...
/**
 * Logique pure de `DateRangePicker` (M1-4, ARCHITECTURE §6.1) : raccourcis de
 * période + grille du mois pour la plage personnalisée. Arithmétique `Date`
 * UTC pure, comme `apps/app/features/calendar/buildCalendarGrid` (pas de
 * dépendance à `date-fns`, non listée dans `packages/ui/package.json`,
 * CLAUDE.md « n'installe aucune dépendance »). Pas encore le vrai « jour de
 * trading » compte par compte (fuseau + heure de bascule,
 * `@repo/core/time#tradingDayOf`) : ce composant manipule des `TradingDay`
 * déjà résolus par l'appelant (`today`), comme `DayCell`/`CalendarScreen`.
 */
...
/** Construit les semaines (7 jours) couvrant `month` (1-12) de `year`, avec bourrage — voir `buildCalendarGrid` (`apps/app`), même algorithme. */
 ...
 * `start` et `end` sont échangés — appuyer avant la borne de départ redémarre
 * proprement plutôt que de produire une plage inversée). Un 3ᵉ appui (plage déjà
 * complète) redémarre une nouvelle sélection à partir de `day`.
 */
