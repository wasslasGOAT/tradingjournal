 ...
 * (M1-6) — format attendu par `recharts` (`LineChart`/`AreaChart` lisent une
 * seule liste de lignes, une clé par série). Union des `x` de toutes les
 * séries, triée croissante ; une série sans point à un `x` donné laisse un
 * trou (`undefined`), pas un `0`.
 */
