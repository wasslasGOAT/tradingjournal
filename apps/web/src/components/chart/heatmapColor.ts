import { hexToRgba } from '@/lib/theme/withAlpha';
import type { ColorTokens } from '@/lib/theme/tokens';

import type { ChartPnlPalette } from './colors';

/**
 * Intensité `[0, 1]` de `value` par rapport à `maxAbsValue` (W-4, copie de
 * `packages/ui/src/chart/heatmapColor.ts`, gelé). Alpha minimal `0.12` même
 * pour la plus petite valeur non nulle : une cellule à très faible intensité
 * doit rester visible/cliquable (contraste), pas quasi transparente.
 * `value === 0` ou `maxAbsValue === 0` -> `0` (case vide, pas de division par
 * zéro).
 */
