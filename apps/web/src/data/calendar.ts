import { aggregateByTradingDay, buildCalendarGrid, computeMonthStats, Decimal, parseAmount } from "@repo/core"
import type { CalendarGridCell, MonthStats, TradeRecord, TradingDay, WeekStartsOn } from "@repo/core"
...
import { SAMPLE_JOURNAL_TRADING_DAYS, sampleTradeRecordsForAccount } from "./sample/tradesSampleData"
 ...
 * appel Supabase).
 */
export async function getCalendarMonthSummary(filters: CalendarFilters): Promise<CalendarMonthSummary> {
  ...
  for (const accountId of accountIds) {
    ...
    for (const day of days) {
      ...
      const pnl = day.tradesCount > 0 ? day.netPnl.plus(existing?.pnl ?? new Decimal(0)) : existing?.pnl ?? null
