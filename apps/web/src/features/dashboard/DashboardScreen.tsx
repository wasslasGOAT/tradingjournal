...
export function DashboardScreen() {
  ...
  const formatEquityYLabel = (y: number) =>
    formatAmount(new Decimal(y), query.data?.currency ?? "USD", { locale, hideAmounts, decimals: 0 })
    ...
    return (
      <div data-testid="screen-dashboard" className="flex flex-col gap-4 p-4 sm:p-6">
        <EmptyState
          ...
          action={{ label: t("dashboard.empty.action"), onClick: () => show(t("header.quickAdd.comingSoon")) }}
        />
      </div>
    )
  }
