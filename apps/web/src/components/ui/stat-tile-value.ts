...
/**
 * Logique pure de `StatTile` (W-4, copie de
 * `packages/ui/src/components/StatTile/statTileValue.ts`, gelé) : quel
 * formateur `@repo/core/format` appeler selon `kind`, et quelle intention P&L
 * (donc quelle classe de couleur) en déduire du signe. Aucun calcul métier
 * ici — uniquement un aiguillage vers les formateurs déjà arrondis/localisés
 * de `packages/core`.
 */
...
/** Intention P&L déduite du signe (zéro -> `flat`, jamais `-0`). */
...
/** Classe Tailwind de couleur du `StatTile` : intention P&L pour montant/pourcentage, neutre pour un simple nombre. */
export function resolveStatTileClassName(kind: StatTileKind, value: Decimal): string {
  return kind === 'number' ? 'text-foreground' : PNL_TEXT_CLASS_NAME[resolvePnlIntent(value)];
}
