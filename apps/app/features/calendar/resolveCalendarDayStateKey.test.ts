...
describe('resolveCalendarDayStateKey', () => {
  it('aujourd’hui prime sur le contenu', () => {
    expect(resolveCalendarDayStateKey('120.00', true, true)).toBe('today');
    expect(resolveCalendarDayStateKey(null, false, true)).toBe('today');
  });

  it('trade positif -> profit, négatif -> loss, nul -> flat', () => {
    expect(resolveCalendarDayStateKey('120.00', false, false)).toBe('profit');
    expect(resolveCalendarDayStateKey('-45.50', false, false)).toBe('loss');
    expect(resolveCalendarDayStateKey('0', false, false)).toBe('flat');
  });
