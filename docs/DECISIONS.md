...
Amendement (2026-09-25, décision utilisateur, W-9) — **changement de thème hors seuil de fluidité** : basculer sombre/clair est une action ponctuelle qui recalcule le style de tout le document (variables CSS réévaluées au changement de `data-theme`), pas une animation. Il **sort** du seuil ≥ 55 fps / aucune image > 50 ms ; exigence propre : **appliqué sans rechargement, en moins de 200 ms**. `Segmented` **reste soumis au seuil**, mesuré sur un usage courant qui ne change pas le thème (bascule $/%/R ou autre `Segmented` de l'app). Mesure qui a motivé l'amendement : `Segmented` pilotant le thème à 47 fps, pire image 83 ms, **après** optimisation réelle (indicateur animé par `transform`) — le coût restant est le recalcul global du style, non l'animation.
Précision (2026-09-25, W-9) — **job de fluidité CI temporairement non bloquant** : le job `e2e-web-perf` (`.github/workflows/ci.yml`) porte `continue-on-error: true`. La mesure **reste exécutée et visible** à chaque run (annotations, résultat du job) ; les seuils ci-dessus ne changent pas.
- Raisons : les mesures sur le poste de dev ne sont pas fiables (CPU déjà occupé à ~30 % par d'autres applications) ; deux boucles de corrections de fluidité ont déjà été faites.
- Mesures actuelles (poste chargé, CPU ×4, `--workers=1`, fps moyen / pire image) : changement de mois 54–57 fps / 50–67 ms ; `Sheet` 48–54 fps / 150–383 ms ; `Segmented` (masquage) 42–46 fps / 83–150 ms ; thème < 1 ms.
- **Condition de retrait** : 3 runs CI consécutifs au seuil, ou décision de l'utilisateur ; `continue-on-error` est alors retiré et le job redevient bloquant. Dette suivie en ROADMAP M1-web.
- `keepPreviousData` est retiré du calendrier (aucune donnée d'un autre mois affichée, exigence « aucune donnée périmée visible » ci-dessus) : mise en conformité, pas d'amendement.
Décision de clôture (utilisateur, 2026-09-26) — **M1-web close avec dette de fluidité** (ROADMAP DW1) : `Segmented`, `Sheet` et changement de mois restent à confirmer en CI ; suivi via le job `e2e-web-perf` (condition de retrait ci-dessus) et le ressenti sur téléphone. Seuils inchangés. Dernier run local du 2026-09-26 (poste chargé par d'autres processus, donc non représentatif) : `Segmented` 32 fps, `Sheet` 45 fps, mois 25 fps ; thème 20 ms (< 200 ms, exigence atteinte).
Réversibilité : facile.
...
Précision (M1-web, 2026-09-26) — **agrégats à série unique** : les agrégats qui renvoient une seule série ou un seul chiffre pour plusieurs comptes (`equityCurveByDayMultiAccount`, `computeLastDayPnl`, `summarizeAccountsOverPeriod`, `packages/core/aggregates/multiAccount.ts`) **ne regroupent pas par devise** : ils appellent `assertSingleCurrency` et lèvent `MixedCurrencyAggregationError` si les comptes ont des devises différentes (erreur typée, jamais un total faux). Les données factices de M1-web sont mono-devise. **Décision ouverte** : comportement de ces vues (dashboard « Tous les comptes », courbe d'equity) quand des comptes de devises différentes arriveront — en M2 (M2-16, vrais comptes) ou au plus tard en M4 (trades réels). Options à instruire alors : une série par devise, restriction « Tous les comptes » à une devise, ou message explicite ; nouvel ADR ou précision datée ici avant le code.
Réversibilité : facile.
...
- Précision (2026-09-25, audit sécurité W-10) — **inscription publique désactivée sur le projet de dev** : son URL et sa clé anon sont désormais publiques dans le bundle de la préproduction (ADR-025). Réglage **par l'utilisateur dans le tableau de bord** (Authentication → « Allow new users to sign up » désactivé), jamais par `config push` ; les testeurs sont ajoutés par « Invite user ». Les redirections d'auth utilisent l'URL **exacte** de l'hôte `edgebook-bs9.pages.dev` (pas de joker).
...
- **Formatage** : le style Prettier du dépôt (`@repo/config/prettier`) s'applique aussi à `apps/web` (reformaté le 2026-09-26, commit `c776f37`).
...
Précision (2026-09-25, W-8/W-10) : préproduction en ligne à `https://wip-m1-m3.edgebook-bs9.pages.dev` (projet Pages `edgebook`). Le bundle expose l'URL et la clé anon du projet Supabase de dev : **l'inscription publique doit y être désactivée** et les testeurs invités (ADR-020, précision du 2026-09-25 — action utilisateur dans le tableau de bord, jamais de `config push`). Redirections d'auth en URL exacte sur l'hôte `edgebook-bs9.pages.dev`.
