...
import { formatStatTileValue, resolvePnlIntent, resolveStatTileClassName } from './stat-tile-value';
...
describe('resolveStatTileClassName', () => {
  it('suit le signe pour amount/signedAmount/percent', () => {
    expect(resolveStatTileClassName('amount', new Decimal('10'))).toBe('text-pnl-profit');
    expect(resolveStatTileClassName('signedAmount', new Decimal('-10'))).toBe('text-pnl-loss');
    expect(resolveStatTileClassName('percent', new Decimal('0'))).toBe('text-pnl-flat');
  });

  it('reste neutre pour un simple nombre', () => {
    expect(resolveStatTileClassName('number', new Decimal('42'))).toBe('text-foreground');
    expect(resolveStatTileClassName('number', new Decimal('-1'))).toBe('text-foreground');
  });
});
