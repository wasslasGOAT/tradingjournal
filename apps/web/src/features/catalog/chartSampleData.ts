...
import type { ChartBarDatum, ChartHeatmapCell } from '@/components/chart';

/**
 * Données factices des sections `Chart`/liste du catalogue (W-4) — copiées
 * (pas importées) depuis l'esprit de
 * `apps/app/features/catalog/chartSampleData.ts` (gelé, ADR-023). Générateurs
 * déterministes (petit générateur pseudo-aléatoire à graine fixe, pas
 * `Math.random`) : le catalogue doit rendre le même graphique à chaque
 * ouverture (revues visuelles, captures d'écran stables).
 */
...
/** ~500 R multiples factices (distribution asymétrique typique) pour l'histogramme (`binNumericValues`). */
