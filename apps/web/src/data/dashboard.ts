...
function computeForAccount(accountId: SampleAccountId, filters: DashboardFilters): PerAccountComputation {
...
export async function getDashboardSummary(filters: DashboardFilters): Promise<DashboardSummary> {
  ...
  const totalStartingBalance = perAccount.reduce((acc, a) => acc.plus(a.startingBalance), new Decimal(0))
  ...
  const returnRate =
    totalStartingBalance.greaterThan(0) ? computeReturnRate(totalStartingBalance, [periodPnl]) : new Decimal(0)
