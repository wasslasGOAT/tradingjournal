...
/**
 * Interface commune de `Chart` (M1-6, ARCHITECTURE §6, ADR-021) : indépendante
 * de la bibliothèque de rendu (`victory-native`/Skia natif, `recharts` web).
 * Aucun calcul métier ici — tous les points sont déjà des `number` (l'appelant
 * convertit les `Decimal` de `@repo/core` en amont) et tous les libellés
 * passent par une fonction fournie par l'appelant (`format*Label`), qui
 * consomme elle-même `@repo/core/format` (CLAUDE.md : pas de calcul/formatage
 * métier dans un composant).
 */
...
/** Point actif (survol web / toucher natif), en coordonnées de domaine (pas en pixels). */
...
/** Contenu de l'état vide — jamais de texte en dur dans `Chart` (i18n, CLAUDE.md) : toujours fourni par l'appelant. */
...
interface ChartCommonProps {
  ...
  /** Requis (pas de texte par défaut ici, CLAUDE.md) : titre/description traduits par l'appelant. */
  ...
  /** Appelé au survol (web) / toucher (natif) — `null` quand aucun point n'est actif. */
