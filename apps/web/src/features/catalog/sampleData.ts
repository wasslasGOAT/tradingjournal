...
/**
 * Données factices du catalogue de composants (W-4, `src/routes/dev/catalog.tsx`)
 * — **copiées** (pas importées) depuis l'esprit de
 * `apps/app/features/catalog/sampleData.ts` (gelé, ADR-023 : `apps/app` n'est
 * pas notre zone). Chaînes décimales volontaires (comme les colonnes
 * `numeric` Postgres lues en `::text`, ADR-005) : converties par
 * `@repo/core#parseAmount` dans les composants, jamais de `number` flottant.
 */

export const SAMPLE_CURRENCY = 'USD';

/** Jour de référence pour la démo `DateRangePicker` — fixe, aperçu stable indépendant de la date réelle. */
...
/** Fraction `[0, 1]` pour une éventuelle barre de progression. */
